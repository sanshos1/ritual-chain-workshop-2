...
contract MockScheduler {
    ...
    function schedule(bytes calldata data, uint32, uint32, uint32, uint32, uint32, uint256, uint256, uint256, address)
...
contract MockRegistry {
    ...
    function configure(address executor_, bool found_) external { executor = executor_; found = found_; }
...
contract MockHttpPrecompile {
    ...
    function configure(uint16 status_, bytes calldata body_, string calldata error_) external {
        status = status_; body = body_; errorMessage = error_;
    }
    fallback() external {
...
contract MockJqPrecompile {
    ...
    function configure(uint256 value_, bool succeeds_) external { value = value_; succeeds = succeeds_; }
    fallback() external {
