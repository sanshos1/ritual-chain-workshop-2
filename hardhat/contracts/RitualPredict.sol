...
contract RitualPredict {
    ...
    /// Let a bettor reduce either side of an open position before betting closes.
    /// Totals are updated before the transfer, so a failed payment cannot corrupt state.
    function withdrawStake(uint256 marketId, bool isYes, uint256 amount) external {
        Market storage m = _market(marketId);
        if (amount == 0) revert ZeroStake();
        if (m.state != MarketState.Open || block.number >= m.closeBlock)
            revert BettingClosed();

        uint256 available = isYes
            ? yesStake[marketId][msg.sender]
            : noStake[marketId][msg.sender];
        if (amount > available) revert InsufficientStake();

        if (isYes) {
            yesStake[marketId][msg.sender] = available - amount;
            m.totalYes -= amount;
        } else {
            noStake[marketId][msg.sender] = available - amount;
            m.totalNo -= amount;
        }

        emit StakeWithdrawn(marketId, msg.sender, isYes, amount);
