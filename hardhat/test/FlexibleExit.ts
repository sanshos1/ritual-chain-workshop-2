...
describe("RitualPredict — Flexible Exit edition", async function () {
  const { viem, networkHelpers } = await network.create();
  const publicClient = await viem.getPublicClient();
  const testClient = await viem.getTestClient();
  const [, yesBettor, noBettor] = await viem.getWalletClients();
  ...
  it("creates a market and schedules autonomous resolution", async () => {
  ...
  it("allows a partial exit and updates the pool before close", async () => {
    const { predict } = await networkHelpers.loadFixture(setup);
    await createMarket(predict);
    await predict.write.bet([1n, true], { account: yesBettor.account, value: parseEther("2") });
    await predict.write.withdrawStake([1n, true, parseEther("0.75")], { account: yesBettor.account });
    const market = await predict.read.getMarket([1n]);
    const stake = await predict.read.yesStake([1n, yesBettor.account.address]);
    assert.equal(stake, parseEther("1.25"));
    assert.equal(market.totalYes, parseEther("1.25"));
  });

  it("rejects exits larger than the bettor position", async () => {
    const { predict } = await networkHelpers.loadFixture(setup);
    await createMarket(predict);
    await predict.write.bet([1n, false], { account: noBettor.account, value: parseEther("1") });
    await assert.rejects(
      predict.write.withdrawStake([1n, false, parseEther("1.01")], { account: noBettor.account }),
    );
  });

  it("locks exits once the betting window closes", async () => {
    const { predict } = await networkHelpers.loadFixture(setup);
    const market = await createMarket(predict);
    await predict.write.bet([1n, true], { account: yesBettor.account, value: parseEther("1") });
    const current = await publicClient.getBlockNumber();
    await testClient.mine({ blocks: Number(market.closeBlock - current) });
    await assert.rejects(
      predict.write.withdrawStake([1n, true, parseEther("0.1")], { account: yesBettor.account }),
    );
  });

  it("resolves YES and calculates payout from the reduced pool", async () => {
    const { predict, scheduler } = await networkHelpers.loadFixture(setup);
    const market = await createMarket(predict);
    await predict.write.bet([1n, true], { account: yesBettor.account, value: parseEther("2") });
    await predict.write.bet([1n, false], { account: noBettor.account, value: parseEther("1") });
    await predict.write.withdrawStake([1n, true, parseEther("1")], { account: yesBettor.account });
    const current = await publicClient.getBlockNumber();
    await testClient.mine({ blocks: Number(market.resolveBlock - current) });
    ...
    assert.equal(stakes[3], parseEther("2"));
  });

  it("invalidates after three oracle failures and exposes a refund", async () => {
